...
function saldoBreakdownPillClass(ms: number): string {
  if (ms > 0) {
    return 'border-emerald-200 bg-emerald-50 text-emerald-900 dark:border-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-100';
  }
  if (ms < 0) {
    return 'border-red-200 bg-red-50 text-red-900 dark:border-red-700 dark:bg-red-950/50 dark:text-red-100';
  }
...
export function RrhhPage() {
  ...
  const deleteEmp = async (id: string, nombre: string) => {
    if (!window.confirm(
      `¿Eliminar definitivamente a ${nombre}?\n\nPreferí usar «Fecha de baja» (Editar) para que deje de figurar desde una fecha y se conserve el historial de fichajes.\nEliminar borra el empleado del padrón (los fichajes quedan huérfanos sin nombre).`,
    )) return;
    try {
  ...
  const exportMonthlyMatrixExcel = async () => {
    ...
    try {
      ...
      const showMonthlyBreakdown = monthlyData.planta === 'tucuman';
      ...
      for (const row of monthlyData.employees) {
        ...
        dataRow.height = showMonthlyBreakdown ? 58 : 46;
        ...
        if (showMonthlyBreakdown) {
          ...
          saldoCell.fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: row.balanceMs < 0 ? 'FFF4CCCC' : 'FFD9EAD3' },
          };
                    ...
                    <table className="min-w-max border-separate border-spacing-1 bg-slate-100 p-1 text-xs dark:bg-slate-900/80">
                      <thead className="text-muted-foreground">
                        <tr>
                          ...
                          <th className="rounded-md bg-slate-200 px-3 py-2 text-right min-w-[112px] text-slate-800 dark:bg-slate-800 dark:text-slate-100">Total trabajado</th>
                          {monthlyData.planta === 'tucuman' && (
                            <>
                          ...
                          <tr key={emp.id} className={`border-b border-border/60 last:border-0 hover:bg-muted/20 ${!emp.activo ? 'opacity-50' : ''}`}>
                            ...
                            <td className="px-4 py-2.5">
                              {isEditing ? (
                                <input
                                  type="date"
                                  value={editEmpDraft.fechaBaja ?? ''}
                                  onChange={(e) => setEditEmpDraft((d) => ({
                                    ...d,
                                    fechaBaja: e.target.value || null,
                                  }))}
                                  className="w-full rounded-md border border-border bg-background px-2 py-1 text-sm"
                                  title="Primer día en que ya no debe figurar. Los meses anteriores se conservan."
                                />
                              ) : emp.fechaBaja ? (
                                <span className="tabular-nums text-muted-foreground">{emp.fechaBaja}</span>
                              ) : (
                                <span className="text-muted-foreground">—</span>
                              )}
                            </td>
